...
void testApp::drawPoses() {
    ...
    if(g_means.size()>0) {
        ...
        // togliendo il for dovrebbe tracckare solo una faccia
          //  for(unsigned int i=0;i<g_means.size();++i){
      //  ofVec3f pos = ofVec3f(g_means[i][0] , g_means[i][1], g_means[i][2]);
        // ofVec3f pos = ofVec3f(g_means[i][0] , g_means[i][1], g_means[i][2]);

                ofVec3f pos = ofVec3f(g_means[0][0] , g_means[0][1], g_means[0][2]);
                ofVec3f dir = ofVec3f(0,0,-150);
               
                dir.rotate(g_means[0][3], g_means[0][4], g_means[0][5]);
          
                dir += pos;
              
                
        if(abs(prevPos.distance(pos)) > 1.5f) 
        {
             lastPos = pos;
             lastDir = dir;
             finalPos = pos;
      ...
      actualDir += (finalDir - actualDir) * .2;
      actualPos += (finalPos - actualPos) * .2;
        
        
        
        ofLine(actualPos.x, actualPos.y, actualPos.z, actualDir.x , actualDir.y, actualDir.z);
        
        
        
        v.setAddress("/htrack/");
        v.addFloatArg(actualPos.x);
        v.addFloatArg(actualPos.y);
        v.addFloatArg(actualPos.z);
        v.addFloatArg(actualDir.x);
        v.addFloatArg(actualDir.y);
        v.addFloatArg(actualDir.z);
...
void testApp::draw(){
	...
	string buf;
	buf = "sending osc messages to" + string(HOST) + "port: " + ofToString(PORT);
